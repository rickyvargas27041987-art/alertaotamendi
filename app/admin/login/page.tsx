  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!data.success) {
        setError(
          data.error ||
            "Contraseña incorrecta o acceso no autorizado."
        );
        return;
      }
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-slate-950 px-4 text-white">
      <div className="w-full max-w-md rounded-[32px] border border-slate-800 bg-slate-900 p-7 shadow-2xl shadow-black/30 sm:p-9">
        <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl bg-red-500 text-3xl shadow-lg shadow-red-950/40">
          🚨
        </div>

        <p className="mt-6 text-center text-[11px] font-bold uppercase tracking-[0.22em] text-red-400">
          Centro de monitoreo
        </p>
        ...
        <p className="mt-2 text-center text-sm text-slate-400">
          Acceso privado para operadores autorizados.
        </p>
        ...
        <form onSubmit={handleSubmit} className="mt-7">
          ...
          {error && (
            <p className="mt-3 rounded-xl bg-red-500/10 p-3 text-center text-sm font-semibold text-red-300">
              {error}
            </p>
          )}
        ...
        <div className="mt-6 border-t border-slate-800 pt-5 text-center text-xs text-slate-600">
          Panel privado · Sesión protegida
        </div>
      </div>
    </main>
  );
